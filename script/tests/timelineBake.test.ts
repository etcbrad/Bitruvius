...
export const runTimelineBakeTests = () => {
  ...
  // Mutate the in-state baseOffset length for this joint to try to trick bakeRecordingIntoTimeline
  // into sampling from `state.joints` instead of the canonical base joints.
  const mutated = {
    ...state,
    joints: {
      ...state.joints,
