...
> = {
  l_elbow: { a: 'l_upper_arm', b: 'l_elbow', c: 'l_wrist', min: 5, max: 175 },
  ...
  neck_base: { a: 'collar', b: 'neck_base', c: 'nose', min: 20, max: 160 },
...
const stepPosePhysicsInternal = (input: PosePhysicsInput): PosePhysicsOutput => {
  ...
  const draggingHeadOrNeck = input.drag?.id === 'nose' || input.drag?.id === 'neck_base';
  if (!draggingHeadOrNeck && rigidity !== 'cardboard' && (bendEnabled || stretchEnabled) && (invMass.collar ?? 1) > 0) {
    ...
    const rShoulderWorld = world0.r_upper_arm;
