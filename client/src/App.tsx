...
import { calculateSegmentScale } from './engine/cutouts';
...
import { BITRUVIAN_CONSTANTS } from './engine/bitruvian/types';
...
import { getOptimalMode, applyPhysicsProfile, shouldAutoSwitch, createSmoothTransition, PHYSICS_PROFILES, type UnifiedPhysicsMode } from './engine/unifiedPhysics';
...
const computeMaxWireStrain = (joints: Record<string, Joint>): number => {
  ...
  let max = 0;
  const missingWireLogged = new Set<string>();
  ...
  for (const w of WIRE_REST_DEFS) {
    ...
    if (!a || !b) {
      const key = `${w.a}-${w.b}`;
      if (!missingWireLogged.has(key)) {
        console.warn(`Missing joints for wire strain calculation: ${w.a} -> ${w.b}`, { w, missingJoints: [w.a, w.b].filter(id => !world[id]) });
        missingWireLogged.add(key);
      }
      continue;
    }
...
export default function App() {
  ...
  if (!initRef.current) {
    ...
    const initialManikinState: SkeletonState = {
      ...initialState,
      // Startup baseline: FK mode (pure forward kinematics).
  ...
  const [skeletonBuilderJson, setSkeletonBuilderJson] = useState(() =>
    JSON.stringify(
      {
        schema: 'bitruvius-skeleton',
        model: initRef.current!.state.activeModel,
        joints: initRef.current!.state.joints,
      },
      null,
      2,
    ),
  );
  const [skeletonBuilderDirty, setSkeletonBuilderDirty] = useState(false);
  const skeletonBuilderFileInputRef = useRef<HTMLInputElement | null>(null);
  ...
  const currentSkeletonBlueprint = useMemo(
    () => ({
      schema: 'bitruvius-skeleton',
      version: 1,
    ...
    [state.activeModel, state.joints],
  );

  const lastSerializedBlueprintRef = useRef<string>('');

  useEffect(() => {
    if (skeletonBuilderDirty) return;
    const newJson = JSON.stringify(currentSkeletonBlueprint, null, 2);
    if (newJson !== lastSerializedBlueprintRef.current) {
      lastSerializedBlueprintRef.current = newJson;
      setSkeletonBuilderJson(newJson);
    }
  }, [currentSkeletonBlueprint, skeletonBuilderDirty]);
  ...
  const [cutoutBuilderOpen, setCutoutBuilderOpen] = useState(false);
  ...
  const nonManikinPinTargetsRef = useRef<{ map: Record<string, Point>; generation: number } | null>(null);
  const engineGenerationRef = useRef(0);
  const lastFkPoseRef = useRef<ReturnType<typeof capturePoseSnapshot> | null>(null);
  ...
  const handleBgVideoMeta = useCallback(
    (meta: ReferenceVideoMeta, expectedSrc: string | null) => {
      ...
      // Always update state if this metadata is for the current source
      if (expectedSrc === state.scene.background.src) {
        setBgVideoMeta(meta);
      }
    ...
    [addConsoleLog, state.scene.background.src],
  );
  ...
  const handleFgVideoMeta = useCallback(
    (meta: ReferenceVideoMeta, expectedSrc: string | null) => {
      ...
      // Always update state if this metadata is for the current source
      if (expectedSrc === state.scene.foreground.src) {
        setFgVideoMeta(meta);
      }
    ...
    [addConsoleLog, state.scene.foreground.src],
  );
    ...
    (actionId: string, update: (prev: SkeletonState) => SkeletonState) => {
      setState((prev) => {
        ...
        if (!Object.is(next, prev)) {
          ...
          const constrainedNext = applyDeactivationConstraints(next);
          if (ENGINE_PERSISTENCE_ENABLED) queueMicrotask(() => queueAutosave(constrainedNext));
          return constrainedNext;
        } else {
          return next;
        }
      });
  ...
  const handleBackgroundFileUpload = useCallback(
    async (file: File) => {
      ...
      if (isGif || isZip) {
        try {
          ...
          const seq = await loadReferenceSequenceFromFile(file, fps, {
            maxFrames: Math.min(state.timeline.clip?.frameCount || 120, REFERENCE_MAX_SECONDS * fps),
          });
          ...
          // Only drop previous sequence after successfully loading new one
          if (prevSeqId) {
            dropReferenceSequence(prevSeqId);
          }
    ...
    [
      addConsoleLog,
      dropReferenceSequence,
      loadReferenceSequenceFromFile,
      referenceSequencesRef,
      setStateWithHistory,
      state.scene.background.sequence?.id,
      state.timeline.clip?.fps,
      state.timeline.clip?.frameCount,
    ],
  );
  ...
  }, [dropReferenceSequence, setStateWithHistory, state.scene.background.sequence?.id]);

  const handleRigModelChange = useCallback(
    (modelId: RigModel) => {
      if (modelId === state.activeModel) return;
      setStateWithHistory('switch_model', (prev) => switchModel(prev, modelId));
      setSkeletonBuilderDirty(false);
      addConsoleLog('info', `Switched rig to ${RIG_MODELS[modelId].name}`);
    },
    [state.activeModel, setStateWithHistory, addConsoleLog],
  );

  const handleBlueprintTextChange = useCallback((event: React.ChangeEvent<HTMLTextAreaElement>) => {
    setSkeletonBuilderJson(event.target.value);
    setSkeletonBuilderDirty(true);
  }, []);

  const applySkeletonBlueprint = useCallback(() => {
    try {
      const parsed = JSON.parse(skeletonBuilderJson);
      const jointSource = parsed && typeof parsed === 'object' ? (parsed.joints ?? parsed) : null;
      if (!jointSource) {
        throw new Error('No joint data provided.');
      }
      const sanitized = sanitizeJoints(jointSource);
      const maybeModel =
        typeof parsed?.model === 'string' && parsed.model in RIG_MODELS ? (parsed.model as RigModel) : null;
      
      if (maybeModel && maybeModel !== state.activeModel) {
        setStateWithHistory('skeleton_blueprint_apply', (prev) => {
          const updated = switchModel(prev, maybeModel);
          return {
            ...updated,
            joints: sanitized,
          };
      ...
      setSkeletonBuilderDirty(false);
      addConsoleLog('success', 'Skeleton blueprint applied');
    } catch (err) {
      const message = err instanceof Error ? err.message : 'unknown error';
      addConsoleLog('error', `Skeleton blueprint load failed: ${message}`);
    }
  }, [skeletonBuilderJson, setStateWithHistory, addConsoleLog]);

  const handleSkeletonBlueprintFileChange = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      file
        .text()
        .then((text) => {
          setSkeletonBuilderJson(text);
          setSkeletonBuilderDirty(true);
          addConsoleLog('info', `Loaded skeleton file ${file.name}`);
        })
        .catch((err) => {
          const message = err instanceof Error ? err.message : 'unknown';
          addConsoleLog('error', `Failed to read skeleton file: ${message}`);
        })
        .finally(() => {
          target.value = '';
        });
  ...
  const handleDownloadSkeletonBlueprint = useCallback(() => {
    const blob = new Blob([JSON.stringify(currentSkeletonBlueprint, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const filename = `bitruvius-skeleton-${currentSkeletonBlueprint.model}-${Date.now()}.json`;
    const anchor = document.createElement('a');
    anchor.href = url;
    anchor.download = filename;
    anchor.click();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
    addConsoleLog('success', `Saved skeleton blueprint to ${filename}`);
  }, [currentSkeletonBlueprint, addConsoleLog]);
  ...
  const updateSheetPalette = useCallback(
    (patch: Partial<SheetPalette>) => {
      setStateWithHistory('sheet_palette_update', (prev) => {
        const basePalette = prev.sheetPalette ?? makeDefaultState().sheetPalette;
        return {
          ...prev,
          sheetPalette: {
            ...basePalette,
  ...
  const assignSegmentToSlot = useCallback(
    (segment: SheetSegment, slotId?: string) => {
      setStateWithHistory('assign_sheet_segment', (prev) => {
        const palette = prev.sheetPalette ?? makeDefaultState().sheetPalette;
        const chooseSlotId = slotId ?? palette.targetSlotId ?? findBestSlotForSegment(prev, segment);
        const assetId = `sheet_segment_${segment.id}_${Date.now()}`;
        const asset: CutoutAsset = {
          ...
          image: {
            ...
            naturalWidth: segment.bounds.width,
            naturalHeight: segment.bounds.height,
          },
        };
        const nextSlots = { ...prev.cutoutSlots };
        if (chooseSlotId && nextSlots[chooseSlotId]) {
          const slot = nextSlots[chooseSlotId];
          const optimalScale = calculateSegmentScale(segment, slot);
          nextSlots[chooseSlotId] = { 
            ...slot, 
            assetId,
            scale: optimalScale // Apply optimal scaling based on joint distance
          };
        }
        return {
          ...prev,
          ...
          sheetPalette: {
            ...
            targetSlotId: chooseSlotId ?? null,
          },
        };
      });
    },
    [findBestSlotForSegment, setStateWithHistory],
  );
  ...
  const setManikinModeEnabled = useCallback(
    (enabled: boolean) => {
      ...
      if (!enabled) {
        // Leaving FK mode -> switch to optimal physics mode
        const fkPose = capturePoseSnapshot(live.joints, 'current');
        lastFkPoseRef.current = fkPose;
        
        clearTransientInteractionState();
        setManikinRotateDragging(null);
        manikinRotateDraggingLiveRef.current = null;
        
        // Use unified physics to determine optimal mode
        const targetMode = shouldAutoSwitch(live, 'exit_fk_mode') ?? 'rigid';
        const transition = createSmoothTransition(live, targetMode, Date.now());
        
        armPoseReliefTransition({ 
          reason: `manikin:off->${targetMode}`, 
          durationMs: transition.transitionConfig.durationMs 
        });
        ...
        if (nonManikinResumeRef.current && nonManikinPinTargetsRef.current &&
          ...
          nonManikinPinTargetsRef.current.generation === engineGenerationRef.current) {
          pinTargetsRef.current = nonManikinPinTargetsRef.current.map;
        }
        
        setStateWithHistory('manikin_mode:off', (prev) => {
          const resume = nonManikinResumeRef.current;
          const upgraded = applyPhysicsProfile(prev, targetMode);
          
          // Apply resume settings if available and compatible
          if (resume && resume._engineGeneration === engineGenerationRef.current) {
            const { _engineGeneration, ...publicResume } = resume;
            Object.assign(upgraded, publicResume);
          } else if (targetMode === 'balanced' || targetMode === 'fluid') {
            upgraded.activeRoots = ['l_ankle', 'r_ankle'].filter((id) => id in prev.joints);
          }
          
          return { ...upgraded, joints: applyPoseSnapshotToJoints(upgraded.joints, fkPose) };
        });
        
        // Wake pose physics for smooth transition
        {
          const nowMs = performance.now();
      ...
      nonManikinResumeRef.current = {
        ...
        activeRoots: [...live.activeRoots],
        rigidity: live.rigidity,
        physicsRigidity: live.physicsRigidity,
        bendEnabled: live.bendEnabled,
        stretchEnabled: live.stretchEnabled,
        leadEnabled: live.leadEnabled,
        snappiness: live.snappiness,
        hardStop: live.hardStop,
        _engineGeneration: engineGenerationRef.current,
      };
      nonManikinPinTargetsRef.current = { map: { ...pinTargetsRef.current }, generation: engineGenerationRef.current };
      pinTargetsRef.current = {};
      armPoseReliefTransition({ reason: 'manikin:on', durationMs: 1600 });
      
      setStateWithHistory('manikin_mode:on', (prev) => {
        const next = applyPhysicsProfile(prev, 'fk');
        const poseToRestore = lastFkPoseRef.current ?? capturePoseSnapshot(prev.joints, 'current');
        lastFkPoseRef.current = null;
        return { ...next, joints: applyPoseSnapshotToJoints(next.joints, poseToRestore) };
      });
      
      addConsoleLog('info', 'FK mode enabled: pure forward kinematics control.');
    },
    [addConsoleLog, clearTransientInteractionState, setStateWithHistory],
  );
  ...
  const exportVideo = useCallback(async (): Promise<void> => {
    ...
    if (!state.timeline.enabled) {
      const message = 'Timeline must be enabled to export video';
      alert(message);
      throw new Error(message);
    }
    if (state.scene.background.mediaType === 'sequence' || state.scene.foreground.mediaType === 'sequence') {
      const message = 'Video export does not support GIF/ZIP sequences yet. Please use a real video file for reference layers, or turn off sequence reference layers before exporting.';
      alert(message);
      throw new Error(message);
    }
    ...
    try {
      setTimelinePlaying(false);
      const success = await exportAsWebm({
        width: canvasSize.width,
        height: canvasSize.height,
        backgroundColor,
      ...
      if (!success) throw new Error('Failed to export video');
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Unknown error';
  ...
  const captureScreenshot = useCallback(() => {
    ...
    // Remove any background rectangles or elements that aren't part of the figure/grid
    const backgroundElements = clone.querySelectorAll('rect[fill*="url"], rect[fill*="#"]');
    backgroundElements.forEach(el => {
      const rect = el as SVGRectElement;
      if (rect.getAttribute('fill')?.includes('ground') || 
          rect.getAttribute('fill')?.includes('system-ground')) {
    ...
    // Convert SVG to data URL and render to canvas
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          a.href = url;
          a.download = `bitruvius-capture-${Date.now()}.png`;
          a.click();
          URL.revokeObjectURL(url);
          addConsoleLog('success', 'Screenshot captured and downloaded');
        }
    ...
    img.src = svgUrl;
  }, [canvasSize, addConsoleLog]);

  // Copy debug console to clipboard
  const copyDebugConsoleToClipboard = useCallback(() => {
    const consoleElement = document.querySelector('.debug-console');
    if (!consoleElement) {
      addConsoleLog('error', 'Debug console element not found');
      return;
    }
    
    const consoleText = consoleElement.textContent || '';
    if (!consoleText.trim()) {
      addConsoleLog('warning', 'Debug console is empty');
      return;
    }
    
    // Create a formatted version with timestamp
    const timestamp = new Date().toISOString();
    const formattedContent = `Bitruvius Debug Console Export\nTimestamp: ${timestamp}\n\n${consoleText}`;
    
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(formattedContent)
        .then(() => {
          addConsoleLog('success', `Debug console copied to clipboard (${consoleText.length} characters)`);
        })
        .catch((err) => {
          addConsoleLog('error', 'Failed to copy debug console to clipboard');
          ...
          addConsoleLog('success', 'Debug console copied to clipboard (fallback method)');
        });
    } else {
      ...
      document.execCommand('copy');
      document.body.removeChild(textarea);
      addConsoleLog('success', 'Debug console copied to clipboard (legacy method)');
    }
  }, [addConsoleLog]);
  const copyAppWindowToClipboard = useCallback(() => {
    // Use html2canvas or similar approach to capture the entire app window
    const appElement = document.body; // Or a more specific app container
    ...
    // Try to use Screen Capture API with proper feature detection
    if (navigator.mediaDevices && navigator.mediaDevices.getDisplayMedia) {
      navigator.mediaDevices.getDisplayMedia({ video: true })
              ...
              if (blob) {
                ...
                if (navigator.clipboard && navigator.clipboard.write) {
                  navigator.clipboard.write([
                    ...
                    addConsoleLog('success', 'App window copied to clipboard');
                  }).catch(() => {
        ...
        .catch(() => {
          addConsoleLog('info', 'Screen capture denied, using fallback');
          captureScreenshot();
        });
    } else {
      // Fallback to regular screenshot
      addConsoleLog('info', 'Screen capture API not available, using fallback');
      captureScreenshot();
    }
  }, [captureScreenshot, addConsoleLog]);

  // State for marking mode
  const [markingMode, setMarkingMode] = useState(false);
  const [markedPoints, setMarkedPoints] = useState<{xNorm: number, yNorm: number}[]>([]);
  const [screenshotForMarking, setScreenshotForMarking] = useState<string | null>(null);

  // Start marking mode with screenshot
  const startMarkingMode = useCallback(() => {
    ...
    img.onload = () => {
      ...
      // Convert to data URL for marking
      const dataUrl = canvas.toDataURL('image/png');
      setScreenshotForMarking(dataUrl);
      setMarkingMode(true);
      setMarkedPoints([]);
      addConsoleLog('info', 'Marking mode enabled - click to add red X marks');
    };
    
    img.src = svgUrl;
  }, [canvasSize, addConsoleLog]);

  // Handle click in marking mode
  const handleMarkingClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    setMarkedPoints(prev => [...prev, { xNorm, yNorm }]);
  }, [markingMode]);

  // Save marked screenshot
  const saveMarkedScreenshot = useCallback(() => {
    if (!screenshotForMarking) return;
    ...
    img.onload = () => {
      ...
      markedPoints.forEach(point => {
        ...
        const xImage = point.xNorm * img.width;
        const yImage = point.yNorm * img.height;
        
        const size = 15;
        ctx.beginPath();
        ctx.moveTo(xImage - size, yImage - size);
        ctx.lineTo(xImage + size, yImage + size);
        ctx.moveTo(xImage + size, yImage - size);
        ctx.lineTo(xImage - size, yImage + size);
        ctx.stroke();
      });
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `bitruvius-marked-${Date.now()}.png`;
          a.click();
          URL.revokeObjectURL(url);
          addConsoleLog('success', 'Marked screenshot saved');
        }
      }, 'image/png');
    };
    
    img.src = screenshotForMarking;
  }, [screenshotForMarking, markedPoints, addConsoleLog]);

  // Exit marking mode
  const exitMarkingMode = useCallback(() => {
    setMarkingMode(false);
    setMarkedPoints([]);
    setScreenshotForMarking(null);
    addConsoleLog('info', 'Marking mode disabled');
  }, [addConsoleLog]);
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (mod) {
        ...
        if (key === 'c') {
          e.preventDefault();
          if (e.altKey) {
            // Option+C: Copy debug console to clipboard
            copyDebugConsoleToClipboard();
          } else if (e.shiftKey) {
            startMarkingMode();
          } else {
            copyAppWindowToClipboard();
          }
          return;
        }
        return;
  ...
  const computedGridOverlayTransform = useMemo(() => {
    ...
    // Unified scaling based on figure height for consistent grid and circle sizing
    const calibratedPxPerUnit = figureHeightPx; // grid totalHeight is 1
    const pxPerUnit = calibratedPxPerUnit;
...
useEffect(() => {
  ...
  const update = () => {
    setState(prev => {
      ...
      if (prev.hipLock?.walkModeEnabled && !prev.timeline.enabled && !isDirectManipulation) {
      ...
      (e: React.MouseEvent) => {
        ...
        if (!manikinMode) {
          if (groundPlaneDragging) {
            ...
            // Ground plane dragging is no longer needed without procgen
            return;
          }
  ...
  const cutoutsLayer = (() => {
    ...
    return items.map((item) => {
      ...
      if (item.kind === 'head') {
        ...
        // Use optimized trapezoid calculation to avoid jitter
        const topWidth = width * 1.2; // Reduced width ratio for stability
        const bottomWidth = width * 0.8; // Reduced taper for stability
        
        // Pre-calculate points to avoid string concatenation jitter
        const halfHeight = height / 2;
        const halfTopWidth = topWidth / 2;
        const halfBottomWidth = bottomWidth / 2;
        
        const trapPoints = [
          `${originX - halfTopWidth},${originY - halfHeight}`,
          `${originX + halfTopWidth},${originY - halfHeight}`,
          `${originX + halfBottomWidth},${originY + halfHeight}`,
          `${originX - halfBottomWidth},${originY + halfHeight}`,
        ].join(' ');
        ...
        <UnifiedPhysicsControl
          ...
          setState={(updater) => {
            if (typeof updater === 'function') {
              applyEngineTransition('physics_update', updater);
            } else {
              applyEngineTransition('physics_update', () => updater);
            }
          }}
          onTransitionStart={(transition) => {
            armPoseReliefTransition({
              reason: transition.transitionId,
              durationMs: transition.transitionConfig.durationMs
            });
          }}
        />
    ...
    <section>
      ...
      <div className="mb-4 space-y-3">
        <MediaUploadPanel
          ...
          description="Upload an image, video, or GIF/ZIP sequence to decorate the scene."
          supportedTypes="image/*,video/*,.zip,application/zip,application/x-zip-compressed"
          currentLabel={
            state.scene.background.sequence?.kind
              ? `${state.scene.background.sequence.kind} sequence`
              : state.scene.background.src
              ? state.scene.background.mediaType.toUpperCase()
              : null
          }
          status={state.scene.background.src ? 'Loaded' : 'Awaiting upload'}
          visible={Boolean(state.scene.background.visible && state.scene.background.src)}
          onUpload={handleBackgroundFileUpload}
  ...
  const ManikinGlobalPanel = () => {
    return (
      <div className="space-y-3 pb-6">
        ...
        <CollapsibleSection title="DragonBones Skeleton Builder" storageKey="btv:manikin:global:skeleton_builder" defaultOpen>
          <div className="space-y-3">
            <div className="text-[10px] text-[#aaa] leading-relaxed">
              Craft or import DragonBones-style blueprints that tie skeleton joints to cutouts/mask assets, then export the result to disk.
              Use the JSON textarea to fine-tune joint positions or load a saved file before clicking Apply.
            </div>
            <div className="space-y-2">
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between gap-3">
                  <span className="text-[10px] uppercase tracking-[0.2em] text-[#777]">Base Rig</span>
                  <select
                    className="bg-[#121212] text-white text-[10px] px-2 py-1.5 border border-white/10 rounded-full"
                    value={state.activeModel}
                    onChange={(e) => handleRigModelChange(e.target.value as RigModel)}
                  >
                    {(Object.keys(RIG_MODELS) as RigModel[]).map((id) => (
                ...
                <div className="flex flex-wrap gap-2 rounded-xl border border-white/10 bg-white/5 p-1.5 text-[10px] tracking-[0.2em] text-[#ccc] justify-start">
                  {(Object.keys(RIG_MODELS) as RigModel[]).map((id) => {
                    const active = state.activeModel === id;
                    return (
                      <button
                        key={`rig-toggle:${id}`}
                        type="button"
                        onClick={() => handleRigModelChange(id)}
                        aria-pressed={active}
                        className={`px-3 py-1 rounded-full font-bold uppercase text-[10px] tracking-[0.25em] transition-all border border-white/15 flex-nowrap ${
                          active
                            ? 'bg-white text-black border-white'
                            : 'bg-transparent text-white/70 hover:text-white hover:border-white/40'
                        }`}
                        title={RIG_MODELS[id].description}
                      >
                        {RIG_MODELS[id].name}
                      </button>
                    );
              ...
              <textarea
                value={skeletonBuilderJson}
                onChange={handleBlueprintTextChange}
                className="w-full min-h-[200px] max-h-[70vh] bg-[#151515] border border-white/10 text-[10px] text-white font-mono rounded-lg p-3 resize-y focus:outline-none focus:border-white/30"
                spellCheck={false}
                placeholder="Paste your skeleton JSON (joints map + optional model)."
              />
            </div>
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={applySkeletonBlueprint}
                className="px-3 py-2 bg-[#1d6ffd] text-[10px] font-bold uppercase tracking-widest rounded-full transition-colors hover:bg-[#3c87ff]"
              >
                Apply Blueprint
              </button>
              <button
                type="button"
                onClick={handleDownloadSkeletonBlueprint}
                className="px-3 py-2 bg-[#222] text-[10px] font-bold uppercase tracking-widest rounded-full border border-white/10"
              >
                Download Blueprint
              </button>
              <button
                type="button"
                onClick={() => skeletonBuilderFileInputRef.current?.click()}
                className="px-3 py-2 bg-[#222] text-[10px] font-bold uppercase tracking-widest rounded-full border border-white/10"
              >
                Load File
              </button>
            ...
            <div className="text-[9px] text-[#555]">
              Saving writes a JSON file that stores joint offsets (optionally with mask references) so the rig can be rebuilt later. Loading accepts exported files or hand-crafted DragonBones-style maps.
            </div>
            <input
              type="file"
              accept=".json,application/json"
              ref={skeletonBuilderFileInputRef}
              className="hidden"
              onChange={handleSkeletonBlueprintFileChange}
            />
          </div>
        </CollapsibleSection>
  ...
  const exportActions = useMemo<ExportAction[]>(() => [
    {
      ...
      onClick: async () => {
        try {
          await copyCurrentPoseCode();
          setExportStatus('Code copied');
        } catch (err) {
    ...
    },
    {
      ...
      onClick: async () => {
        try {
          ...
          setExportStatus('State downloaded');
        } catch (err) {
    ...
    },
    {
      ...
      onClick: async () => {
        try {
          await exportGif();
          setExportStatus('Exported ZIP');
        } catch (err) {
          const message = err instanceof Error ? err.message : 'Failed to export GIF';
          setExportStatus(`Failed: ${message}`);
        }
      },
    },
  ], [copyCurrentPoseCode, downloadStateJson, exportPng, exportSvg, exportVideo, exportGif, setExportStatus]);
  ...
  return (
    <TooltipProvider delayDuration={200}>
      ...
      >
        <div className="w-[360px] h-full flex flex-col">
          <div className="p-6 pb-4">
            ...
            >
              <button
                ...
                title={manikinMode ? 'Switch to IK mode' : 'Switch to FK mode'}
              >
                    ...
                    <input
                      ...
                      className={`w-16 px-2 py-1 rounded bg-[#111] border border-white/10 text-[10px] font-mono ${
                        state.hipLock?.walkModeEnabled ? 'text-white' : 'text-[#555]'
	                      ...
	                      title="Hip walk oscillation amount"
	                    />
	                  </div>
          ...
          <div className="p-6 pt-0 space-y-3">
            <ExportActionsPanel actions={exportActions} lastStatus={exportStatus ?? undefined} />
      ...
      {/* Main Viewport */}
      <main
        className="flex-1 relative z-0 flex overflow-hidden min-h-0 min-w-0"
      ...
      >
        ...
        {/* Toggle Right Console Button */}
        <button 
          onClick={() => setRightConsoleOpen(!rightConsoleOpen)}
          className="absolute top-6 right-6 z-10 p-2 bg-[#121212] border border-[#222] rounded-lg hover:bg-[#222] transition-colors"
        >
          {rightConsoleOpen ? <ChevronRight size={16} /> : <ChevronLeft size={16} />}
        </button>
        ...
        <CutoutBuilderErrorBoundary
          onError={(error, errorInfo) => {
            console.error('CutoutRigBuilder error:', error, errorInfo);
            // Optionally send error reporting here
          }}
        >
          <CutoutRigBuilder
            open={cutoutBuilderOpen}
            onClose={() => setCutoutBuilderOpen(false)}
            sheetPalette={state.sheetPalette}
            updateSheetPalette={updateSheetPalette}
            assignSegmentToSlot={assignSegmentToSlot}
            setStateWithHistory={setStateWithHistory}
  ...
  {markingMode && (
    <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center">
      ...
      >
        <img 
          src={screenshotForMarking || ''} 
          alt="Screenshot for marking" 
      ...
      {/* Controls */}
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex gap-4 bg-black/90 rounded-lg p-4">
        <button
          onClick={saveMarkedScreenshot}
          className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-bold text-sm"
        >
          Save Screenshot
        </button>
        <button
          onClick={exitMarkingMode}
          className="px-6 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold text-sm"
        >
          Cancel
        </button>
        <div className="flex items-center text-white text-sm">
          Click to add red X marks ({markedPoints.length} placed)
        </div>
      </div>
    </div>
  )}
